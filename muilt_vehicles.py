...
OUTPUT_FOLDER = os.path.join("/mnt/ssd/dataset/sequences/"+id, PHASE)
folders = ['calib', 'image_2','labels', 'label_2', 'velodyne', 'planes']
...
SEM_LIDAR_PATH = os.path.join(OUTPUT_FOLDER, 'labels/{0:06}.label')
...
class SynchronyModel(object):
    ...
    def _span_player(self):
        ...
        my_vehicle_bp = random.choice(self.blueprint_library.filter("vehicle.audi.a2"))
        spawn_points = self.map.get_spawn_points()
        spawn_point = spawn_points[0]  # Same start point for ever simulation
        my_vehicle = self.world.try_spawn_actor(my_vehicle_bp, spawn_point)

        # location = carla.Location(190, 10, 2)
    ...
    def _span_sensor(self, player):
        ...
        lidar_bp.set_attribute('range', '230.0')
        lidar_bp.set_attribute('rotation_frequency', '20')
        lidar_bp.set_attribute('upper_fov', '10')
        lidar_bp.set_attribute('lower_fov', '-30')
        lidar_bp.set_attribute('points_per_second', '2930000')
        lidar_bp.set_attribute('channels', '80')

        # sem_lidar_bp.set_attribute('range', '230.0')
        # sem_lidar_bp.set_attribute('rotation_frequency', '20')
        # sem_lidar_bp.set_attribute('upper_fov', '10')
        # sem_lidar_bp.set_attribute('lower_fov', '-30')
        # sem_lidar_bp.set_attribute('points_per_second', '1440000')
        # sem_lidar_bp.set_attribute('channels', '80')
        ...
        # my_sem_lidar = self.world.spawn_actor(sem_lidar_bp, transform_sensor, attach_to=player)
        ...
        # self.sensors.append(my_sem_lidar)
    ...
    def _save_training_files(self, datapoints, semantic_point_cloud):
        """ Save data in Kitti dataset format """
        # print("Attempting to save no:" + str(self.captured_frame_no))
        groundplane_fname = GROUNDPLANE_PATH.format(self.captured_frame_no)
        lidar_fname = LIDAR_PATH.format(self.captured_frame_no)
        sem_lidar_fname =  SEM_LIDAR_PATH.format(self.captured_frame_no)
        ...
        save_lidar_data(lidar_fname,sem_lidar_fname, semantic_point_cloud)
    ...
    with SynchronyModel() as sync_mode:
        # try:
            ...
            while True:
                ...
                #datapoint : kitti format 3d bounding box descriptor
                if datapoints and step % 3 == 0:
                    data = np.copy(np.frombuffer(sync_mode.point_cloud.raw_data, dtype=np.dtype('float32')))
                    data = np.reshape(data, (int(data.shape[0] / 6), 6))
                    for i in range(len(sync_mode.point_cloud)):
                        data[i,5]=str(sync_mode.point_cloud[i].object_tag)
                    # Isolate the 3D data
                    # points = data[:, :-1]
                    ...
                    sync_mode._save_training_files(datapoints, data)
                    print("Attempting to save no: {0:06}".format(sync_mode.captured_frame_no))
